...
class ContentContainer extends Component {
    ...
    resetEditorOnUpdate = () => {
        ...
        if(currentImageData !== this.state.originalImageData) {
            let removedImageFromOriginal = this.state.originalImageData ? this.state.originalImageData.filter(item => currentImageData.includes(item) === false) : [];
    ...
    resetEditor = () => {
        ...
        if(originalImageData !== this.state.currentImageData) {
            let removedImageNotOriginal = this.state.currentImageData ? this.state.currentImageData.filter(item => originalImageData.includes(item) === false) : [];
    ...
    handleDeleteImage = (imageData) => {
        ...
        var removedImages = oldImageData ? oldImageData.filter(item => imageData.includes(item) === false) : [];
        var removedImagesNotOriginal = removedImages ? removedImages.filter(item => this.state.originalImageData.includes(item) === false) : [];
    ...
    render() {
        return (
            <div>
                <ReactQuill ref={this.reactEditor} theme="snow" modules={this.modules} readOnly={true}
                    formats={this.formats} 
                    onBlur={this.handleQuillBlur}
                    onChange={this.rteChange} 
                    defaultValue={this.state.content || ''} className="exam-content"/>
